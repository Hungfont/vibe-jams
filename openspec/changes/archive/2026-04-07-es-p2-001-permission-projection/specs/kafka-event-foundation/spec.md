...
The system SHALL provision `jam.session.events`, `jam.queue.events`, `jam.playback.events`, `jam.moderation.events`, `jam.permission.events`, and `analytics.user.actions` with partition and retention settings aligned to defined runtime baseline.
...
The system SHALL publish jam session, queue, playback, moderation, and permission events using `sessionId` as the Kafka key and SHALL publish analytics events using `userId` as the Kafka key.
