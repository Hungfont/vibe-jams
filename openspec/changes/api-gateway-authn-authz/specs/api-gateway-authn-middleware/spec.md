...
The api-gateway SHALL extract the Bearer token from the `Authorization` header and call `POST /internal/v1/auth/validate` on auth-service to validate it for every request not matching the public route bypass list.

#### Scenario: Valid token on protected route
- **WHEN** a client sends a request to a protected route with a valid Bearer token
...
- **WHEN** a client sends a request to a protected route without an `Authorization` header
