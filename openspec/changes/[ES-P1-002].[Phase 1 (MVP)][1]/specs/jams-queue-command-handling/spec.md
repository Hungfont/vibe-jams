...
The `jams` service SHALL apply add, remove, and reorder operations as atomic mutations and SHALL increment `queueVersion` by exactly 1 for each successful mutation.

#### Scenario: Successful add increments version once
- **WHEN** a client sends a valid add command for a jam session queue
...
- **WHEN** a client sends a valid remove command for an existing queue item
