...
ALWAYS APPLY FOR backend folder if 
# TDD Command
...
- ALWAYS APPLY: If the role is specified as Frontend Engineer, then apply the rules to frontend code.
- Otherwise, if the role is Backend Engineer, then apply the rules to backend code.
- Exceptions apply only when a different agent is explicitly specified.
