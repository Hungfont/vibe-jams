...
export async function POST(
  ...
  const result = await backendJson<SessionSnapshot>({
    service: "jam",
    path: `/api/v1/jams/${encodeURIComponent(jamId)}/moderation/mute`,
    method: "POST",
    body: parsed.data,
  ...
  if (!result.ok || !result.data) {
    return jsonError(result.error?.code ?? "internal_error", result.error?.message ?? "failed to mute participant", result.status);
  }
