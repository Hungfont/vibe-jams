...
import { queueRemoveSchema } from "@/lib/jam/schemas";
...
export async function POST(
  ...
  const parsed = queueRemoveSchema.safeParse(body);
  if (!parsed.success) {
    return jsonError("invalid_input", "invalid remove payload", 400);
  }

  const { jamId } = await context.params;
  const result = await backendJson<QueueSnapshot>({
    service: "jam",
    path: `/api/v1/jams/${encodeURIComponent(jamId)}/queue/remove`,
    method: "POST",
    body: parsed.data,
  ...
  if (!result.ok || !result.data) {
    return jsonError(result.error?.code ?? "internal_error", result.error?.message ?? "failed to remove queue item", result.status);
  }
