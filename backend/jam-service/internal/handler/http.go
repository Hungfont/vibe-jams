...
func writeServiceError(w http.ResponseWriter, err error) {
	...
	case service.IsTrackRestricted(err):
		apierror.Write(w, http.StatusForbidden, apierror.CodeTrackRestricted, "track restricted")
...
// It first tries gateway-injected X-Auth-* headers (zero-cost path), then
// falls back to the configured Validator (local JWT or HTTP).
