...
import (
	...
	"video-streaming/backend/playback-service/internal/server"
)

// main boots playback-service HTTP server with graceful shutdown.
func main() {
	cfg, err := config.Load()
	if err != nil {
	...
	go func() {
		slog.Info("starting playback-service", "addr", addr)
	...
	slog.Info("playback-service shut down cleanly")
}
